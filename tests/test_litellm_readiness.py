...
nothing is the worst failure mode this project can have. `commons_tier` —
renamed `portage_tier` with the settled module name (2026-07-28, CW02 §2.2) —
fixed that specific defect; this test guards against the *class* of regression
by asserting the actual deployment count a config loads with, not just that the
YAML parses.
